...
import {
  ...
  oneAllocation,
} from '../services/arriveShop';

export default {
  namespace: 'arriveShop', // 新零售常规组IS
  ...
  effects: {
    ...
    * editChanceDetail({ payload }, { call, put, select }) { //编辑信息
      const detail =  yield select(state => state.arriveShop.currentDetail);
      const { stores, contacts, ...chance} = detail;
      const params = {
        chance: {
          ...chance,
          ...payload.chance,
      ...
      if (res.code === 0) { // 成功
        ...
        yield put(routerRedux.push(`/arriveShop/chanceDetail/${detail.id}/basic`));
        return true
      }
      return false;
    },
    ...
    * queryRecords({ payload }, { call, put, select }) { // 请求日志, 根据tab判断
      const { tab } = payload;
      const [id] = yield select(state => [state.arriveShop.id]);
    ...
    * checkCurrentChance({ payload }, { put }) { // 跳转机会详情
      ...
      yield put(routerRedux.push(`/arriveShop/chanceDetail/${id}/basic`));
    },
    ...
    * editCurrentPushOrder({ payload }, { put }) { // 跳转编辑提单详情
      const { pushOrderID } = payload;
      yield put({
        type: 'updateState',
        payload: { pushOrderID },
      });
      yield put(routerRedux.push(`/arriveShop/pushOrderEdit/${pushOrderID}`));
    },
    * pushOrder({ payload }, { put }) { // 跳转提单页面
      const { id } = payload;
      yield put(routerRedux.push(`/arriveShop/pushOrder/${id}`));
    },
