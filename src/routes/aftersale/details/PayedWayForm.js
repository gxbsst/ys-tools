  ...
  showModal(type, data) {
    switch (type) {
      case 'new':
        this.setState({
          showModal: true,
          currentSelect: {...emptyContactItem, isNew: true, key: `NEW_TEMP_ID_${this.index}`}
        });
        this.index += 1;
        return;
      default:
  ...
  render() {
    ...
    return (
      <div>
        ...
        <PayedWayModal item={this.state.currentSelect} onOK={this.saveRow.bind(this)} {...modalProps}
                       amount={this.state.business.amount}/>}
      </div>
    );
  }
}
