...
@connect(state => (
  {
    ...
    submitting: state.loading.effects['arriveShop/addVisitRecord'],
  }
))
@Form.create()
  ...
  render() {
    ...
    const handleOk = () => {
      validateFields((errors, fieldsValue) => {
        ...
        dispatch({
          type: 'arriveShop/addVisitRecord',
          payload: { ...data },
        }).then((val) => {
          if (val) {
            this.props.onOk();
            ...
            <Col>
              <Form.Item label="拜访时间" {...itemLayout}>
                {getFieldDecorator('visitTime', {
                  rules: [{ required: true, message: '请输入拜访时间', transform: (val) => val && val.format('YYYY-MM-DD') }],
                })(
                  <DatePicker />
                )}
              </Form.Item>
