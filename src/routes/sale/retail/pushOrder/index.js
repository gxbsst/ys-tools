...
@connect(({ saleRetail, loading, user }) =>
  ({ saleRetail, submitting: loading.effects['saleRetail/confirmPushOrder'], user })
)
@Form.create()
export default class PushOrder extends PureComponent {
  ...
  componentDidMount() {
    ...
    this.props.dispatch({
      type: 'saleRetail/queryPushOrderInfo',
      payload: {
  ...
  render() {
    ...
    const { id, currentPushOrder } = this.props.saleRetail;
