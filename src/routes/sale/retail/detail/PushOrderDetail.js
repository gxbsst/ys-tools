...
@connect(({ saleRetail, loading, user }) =>
  ({
    records: saleRetail.pushOrderRecords,
    id: saleRetail.id,
    currentDetail: saleRetail.currentDetail,
    user,
    loading: loading.effects['saleRetail/queryRecords'],
  })
)
export default class PushOrderDetail extends PureComponent {
  ...
  handleMenuClick(record, type) {
    ...
    switch (type) {
      case 'push':
        this.props.dispatch({
          type: 'saleRetail/pushCurrentPushOrder',
          payload: { id },
        });
        break;
      case 'detail':
        this.props.dispatch({
          type: 'saleRetail/checkCurrentPushOrder',
          payload: { id },
        });
        break;
      case 'edit':
        this.props.dispatch({
          type: 'saleRetail/editCurrentPushOrder',
          payload: { id },
        });
        break;
      case 'remove':
        this.props.dispatch({
          type: 'saleRetail/removeCurrentPushOrder',
          payload: { id },
        });
        break;
    }
