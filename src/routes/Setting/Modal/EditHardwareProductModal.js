...
import { Form, Input, InputNumber, Radio, Select, Modal, Row, Col, Spin } from 'antd';
...
@Form.create()
export default class EditProductsHardwareModal extends PureComponent {
  state={
    chargeMode: 2,
    product: {},
  }
            ...
            <Col span={16}>
              <Form layout="horizontal" hideRequiredMark>
                ...
                { /*getFieldDecorator('chargeMode', {
                  initialValue: product.chargeMode || this.state.chargeMode,
                ...
                <Row>
                  <Col span={12}>
                    ...
                    >
                    {
                      ...
                      rules: [{ required: true, pattern: /^\d{1,8}(\.\d*$|$)/, message: `请输入正确的${productFieldLabels.price}` }],
                    })(
                  ...
                  <Col span={12}>
                    ...
                    >
                    {
                      readOnly ? product.costPrice :
                      getFieldDecorator('costPrice', {
                      initialValue: product.costPrice,
                      rules: [{ required: true, pattern: /^\d{1,8}(\.\d*$|$)/, message: `请输入正确的${productFieldLabels.costPrice}` }],
                    })(
                      <Input type="number" step="0.01" min="0.01" />
                    )}
                    </Form.Item>
                ...
                <Row>
                  <Col span={12}>
                    ...
                    >
                    {
                      ...
                      rules: [{ required: true, pattern: /^\d{1,8}(\.\d*$|$)/, message: `请输入正确的${productFieldLabels.servicePrice}` }],
                    })(
                      <Input type="number" step="0.01" min="0.01" />
                    )}
                    </Form.Item>
