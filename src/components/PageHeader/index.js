...
function getBreadcrumb(breadcrumbNameMap, url) {
  ...
  Object.keys(breadcrumbNameMap).forEach((item) => {
    ...
    // const itemRegExpStr2 = item.replace(/\/:[\w-]+/g, '');
    // if (itemRegExpStr2 === url) {
    ...
    if (itemRegExp.test(url)) {
      breadcrumb = breadcrumbNameMap[item];
      // console.info(url.match(itemRegExp), url);
      // const matchStr = url.match(itemRegExp)[1];
      // if (matchStr) {
      //   breadcrumb = { name: matchStr };
      // }
    }
  });
  return breadcrumb;
...
export default class PageHeader extends PureComponent {
  ...
  render() {
    ...
    if (routes && params) {
      breadcrumb = (
        <Breadcrumb className={styles.breadcrumb} routes={routes.filter(route => route.breadcrumbName)} params={params} itemRender={this.itemRender}/>
      );
    ...
    return (
      <div className={clsString}>
        ...
        <div className={styles.detail}>
          ...
          <div className={styles.main}>
            <div className={styles.row}>
              {title && <h1 className={styles.title}>{typeof title === 'string' ? title :
                <Row gutter={8}>{title.map((item, index) =>
                  <Col span={~~(24 / title.length)} key={index}>{item}</Col>)}</Row>}</h1>}
            </div>
            <div className={styles.row}>
        ...
        </div>
        {
          tabList &&
          tabList.length &&
          <Tabs {...activeKeyProps} className={styles.tabs} animated={false} onChange={this.onChange}>
