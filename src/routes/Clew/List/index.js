...
function getMultiRole(permissions) {
  const multiPermission = permissions.filter((i) => [3001000, 4001000, 4002000, 5005000, 5006000].includes(i));
...
function getMultiTerm(permissions, multiRoleOne) {
  return permissions.filter((i) => {
    return [3001000, 4001000, 4002000, 5005000, 5006000].includes(i) &&
    Math.floor(i / 10000) === multiRoleOne / 10000 &&
    i !== multiRoleOne;
  })
}
...
@Form.create()
@can([3001000, 4001000, 4002000, 5005000, 5006000], true)
export default class ClewList extends PureComponent {
  ...
  render() {
    ...
    const action = (
      <React.Fragment>
        <div style={{ float: 'left' }}>
          {`-${roleMap[multiRoleOne]}${[4000000, 5000000].includes(multiRoleOne) ? `-${{...teamMap[400], ...teamMap[500]}[multiTermOne]}` : ''}`}
        </div>
        {
          [4000000, 5000000].includes(multiRoleOne) && multiTerm && Array.isArray(multiTerm) && multiTerm.length > 1 &&
          <RadioGroup onChange={this.changeMultiTerm} style={{ marginRight: 30 }} defaultValue={multiTerm[0]} value={this.state.multiTermOne}>
            {Object.entries(teamMap[multiRoleOne / 10000]).map(([k, v]) => <Radio key={k} value={~~k}>{v}</Radio>)}
          </RadioGroup>
        }
        {
    ...
    const extra = (
      [3001001, 4001001, 4001004, 4002001, 5005001, 5006001].includes(~~key) &&
      <InputGroup compact>
        ...
        <Card bordered={false}>
          ...
          <div className={styles.tableList}>
            ...
            <div className={styles.tableListOperator}>
              ...
              {
                ['3001002', '4001003', '4002002', '5005001'].includes(key) &&
                <Button type="default" disabled={!isAbled} onClick={this.showAssignModal}>分配</Button>
              }
              {
                ['5006001'].includes(key) &&
                permissions.includes(5006002) &&
                <React.Fragment>
